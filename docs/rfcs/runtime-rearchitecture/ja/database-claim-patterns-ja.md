...
実際には、通常以下のようなパターンに落ち着く:

- `FOR UPDATE SKIP LOCKED`
- 順序付きで candidate を1件選択
- 同じ transaction 内で guarded update を実行

PostgreSQL は concurrent worker に対する row-locking model が明快なため、この形がよく機能する。

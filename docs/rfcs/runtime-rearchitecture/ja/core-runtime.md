...
2つのワーカーが競合した場合、リースを受け取れるのは1つだけです。
...
- run がまだ leased 状態である
- そのワーカーがリースを所有している

これにより、古いワーカーが、すでに別のワーカーに回収された run を延長したり完了したりすることを防げます。

### 4. 完了または失敗

完了と失敗は所有権チェック付きの書き込みです。

ランタイムは、現在のワーカーがまだリースを所有している場合にのみ、run を `leased` から `completed` または `failed` に遷移させます。
...
チェックポイントの削除はデフォルトの実行パスに含めるべきではありません。

デフォルトの振る舞いとしてステップ履歴を保持します。再開可能性と監査可能性がそれに依存しているためです。保持とクリーンアップは、メンテナンスジョブや時間ベースのポリシーなどで明示的に処理します。

## アーキテクチャの分離

現在の実装は、ランタイムセマンティクス・ポーリング動作・Kysely ベースの永続化を密結合しすぎています。

目標アーキテクチャではそれらを5つのレイヤーに分離します。
...
キューストアは run のライフサイクルとリースセマンティクスを管理します。
...
チェックポイントストアはステップの永続化・進捗・ログを管理します。
...
この分離は意図的です。リースの所有権とチェックポイントの永続化は異なる理由で進化するため、1つの肥大化したストレージインターフェースで結合すべきではありません。
...
これはキューストアの取得およびリース更新操作によって保証します。

### 同じ種類の run を同時に実行しない

一部のジョブは、異なる run であっても同時に実行すべきではありません。

これは取得ロジックの責務です。キューストアは `concurrencyKey` を共有する run を除外またはシリアライズできる必要があります。
...
- 古い完了とリース更新を拒否する
- 期限切れの run を claim セマンティクスにより自動的に回収する
...
8. 所有権チェック付きの書き込みに `workerId` が必要になる

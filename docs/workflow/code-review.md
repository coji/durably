...
4. If the result is `NO-GO`, the orchestrator fixes every blocker, runs focused checks plus the repository validation command, commits, and pushes. Start a new review round for the new head. The first round reviews the full `base...HEAD` diff; later rounds review the cumulative delta from the last completed full-diff review, its affected callers and invariants, and the full trigger set of every unresolved blocker. Carry forward the previous round's verified findings and coverage, marking which acceptance checks were reverified and which were carried forward. Repeat the full-diff review when the fix changes the feature's scope or architecture, or invalidates the earlier coverage. Do not reuse a previous GO. [ADR 0009](../adr/0009-cumulative-delta-review-rounds.md) records this coverage decision.

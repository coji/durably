...
- **Baseline check.** With `"baselineCheck": true` in `factory.json`, the pinned check runs once on the base commit, in the freshly set up worktree, before any agent call. It is off by default. It uses the same checkpoint pair as verification, so a completed result is read back on resume and a check that was cut off is graded again. A failing check stops the run as `baseline-check-failed`. So do a check that cannot start, and setup or a check that leaves changes to tracked files. With the baseline check on, setup must not leave untracked files or directories that `.gitignore` does not cover, empty directories included, because the clean after the check would remove them. This is checked at the end of the setup step, before the check runs, with a dry run of the same clean (`git clean -ffdn`), so the check and the clean always agree on what counts: an empty directory and a nested repository do, a directory that holds only ignored files does not; if any exist, the run stops as `baseline-check-failed`, names the first few paths, and tells the person to add them to `.gitignore` or turn `baselineCheck` off. After a passing check, `git clean -ffd` removes every untracked file that is not ignored, nested repositories included, so the check's output is never sealed into the first candidate. Because of the setup rule, that removes only what the check wrote, including what an interrupted attempt left before a resume re-graded the base. Ignored files, such as installed dependencies, stay. If anything that is not ignored is still untracked after the clean, the run stops as `baseline-check-failed` instead of sealing it.
- **Preflight.** After the baseline and before the first agent call, including triage, every role's provider, model and effort is checked. Each distinct setting is checked once. A free check comes first (for Codex, `model/list` through the run's CLI). Only a setting the free check cannot decide gets one minimal paid call. That call goes through the same checkpoint and measurement path as every other call, so its usage is reported. A refusal is an explicit answer from the provider: a 4xx, a CLI that never started, or an authentication or model-not-found error. A refusal is recorded as a completed call, and the run stops as `preflight-failed`, which is safe to retry. A start handshake that only timed out is not a refusal, because the CLI may just be slow to come up: the free check leaves the setting undecided, and the minimal call decides. The uncertain-call rule does not change: a minimal call with a start checkpoint and no completed one, including one whose own handshake timed out, stops the run as `uncertain-invocation` and is never sent again. A model missing from `model/list` does not prove anything, because the list leaves out hidden models, so the minimal call decides.
...
### Keeping setup's untracked output and removing only what the check added

Rejected. Comparing untracked file names before and after the check kept producing gaps: a resumed baseline counted an interrupted attempt's output as setup's, a check that rewrote a setup file kept its changed bytes, a long listing could be cut short, and a nested repository could not be removed file by file. Requiring setup to leave no such files makes a plain `git clean -ffd`, checked afterwards, correct.

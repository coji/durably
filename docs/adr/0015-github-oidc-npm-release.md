...
- Dependency installation and package builds cannot access npm publishing or GitHub write credentials. The publishing job is independently restricted to `main` by the GitHub Environment and cannot write repository contents. The GitHub Release job cannot request npm OIDC credentials.

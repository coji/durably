...
Install dependencies, build, and create the final tarballs in a read-only job. Add the release commit as `gitHead` to those tarballs for retry verification. Transfer them through a digest-verified GitHub Actions artifact to a separate job that alone has npm OIDC and GitHub contents write permissions. Publish the core tarball before the React tarball; skip an already published version only if its npm `gitHead` is this commit. Create the GitHub Release and tag only after both packages are published.
...
- Dependency installation and package builds cannot access npm publishing or GitHub write credentials.

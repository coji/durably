...
Install dependencies, build, and create the final tarballs in a read-only job. Add the release commit as `gitHead` to those tarballs for retry verification. Transfer them through a digest-verified GitHub Actions artifact to a publish job protected by a main-only GitHub Environment. Publish the core tarball before the React tarball; skip an already published version only if its npm `gitHead` and registry integrity match this commit and tarball. Do not persist checkout credentials in this job, and expose its GitHub token only to the final step that creates the GitHub Release and tag after both packages are published.
...
- Dependency installation and package builds cannot access npm publishing or GitHub write credentials. The publishing job is independently restricted to `main` by the GitHub Environment, and the npm subprocess does not receive the GitHub token used by the final Release step.

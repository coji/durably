...
PostgreSQL active triggers use the same transaction-scoped advisory lock domain as worker claims. Every keyed batch acquires its distinct concurrency keys in sorted order before processing items, including batches without active coalescing; otherwise a non-active batch can deadlock with an active batch through pending-run unique-index waits. Workers use `pg_try_advisory_xact_lock`, skip unavailable keys for the current poll, and reconsider them on later polls. This avoids waiting for one key while retaining another key lock.

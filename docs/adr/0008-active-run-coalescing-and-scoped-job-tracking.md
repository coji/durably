...
PostgreSQL active triggers use the same transaction-scoped advisory lock domain as worker claims. Every keyed batch hashes its concurrency keys, deduplicates and sorts the resulting advisory lock IDs, and acquires them in that order before processing items, including batches without active coalescing. Ordering the original strings is insufficient because distinct strings can collide under `hashtext()` and reverse the lock order across batches. Workers use `pg_try_advisory_xact_lock`, skip unavailable keys for the current poll, and reconsider them on later polls. This avoids waiting for one key while retaining another key lock.

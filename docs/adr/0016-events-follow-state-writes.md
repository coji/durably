...
- Terminal checkpoint cleanup (`preserveSteps: false`) starts after the event for every terminal state that cleans up; a run that ends failed because a `step.all()` failed after another branch succeeded keeps its checkpoints (ADR-0005). A cleanup failure after a committed cancellation is reported as `worker:error` rather than rejecting `cancel()`.

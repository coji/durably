...
- Everything else is unchanged. A call whose outcome is unknown — a start-only checkpoint, a timeout, a cancel, or an error the provider rules do not recognise as a refusal — still stops as `uncertain-invocation` and is never resent automatically.
...
- The refusal rules now decide retryability for every stage, so a wrong rule would make an uncertain call look safe to retry. The rules only match explicit refusals the providers report, and any unrecognised error stays uncertain.

...
The workflow refuses to publish from a non-main ref, from a commit that is no longer main when publishing starts, before successful main CI, when package versions, the React peer range, or changelog disagree, or after the release tag exists. A read-only build job installs dependencies and produces the two inspected tarballs. A main-protected publish job receives those artifacts and publishes the core package and then the React package through npm OIDC. A small final job has GitHub write permission without npm OIDC and creates the GitHub Release after both packages succeed.

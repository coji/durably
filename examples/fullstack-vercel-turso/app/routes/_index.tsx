...
import { ReportForm } from './_index/report-form'
import { ReportProgress } from './_index/report-progress'
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === 'report') {
    ...
    return { intent: 'report', runId: run.id }
  }
...
export default function Index() {
  const [activeJob, setActiveJob] = useState<'image' | 'sync' | 'report'>(
    'report',
  )
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-6xl p-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            <section className="rounded-lg bg-white p-6 shadow">
              ...
              {activeJob === 'image' && <ImageProcessingForm />}
              {activeJob === 'sync' && <DataSyncForm />}
              {activeJob === 'report' && <ReportForm />}
            </section>

            {/* Progress Display */}
            {activeJob === 'image' && <ImageProcessingProgress />}
            {activeJob === 'sync' && <DataSyncProgress />}
            {activeJob === 'report' && <ReportProgress />}
          </div>
        ...
        <footer className="mt-8 text-center text-sm text-gray-500">
          <p>
            Data is stored in Turso (libSQL). Worker runs via onRequest + Vercel
            Cron.
          </p>
          <p className="mt-1">

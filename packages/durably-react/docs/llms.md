...
`isWaiting` is true exactly when status is `waiting`. A signal or durable deadline can finalize the wait result while the run remains `waiting` for an available worker and any valid same-concurrency-key lease. The hook changes to `leased` on resume and then tracks the job's signal or timeout branch; finalizing the wait alone does not make the run terminal. `isActive` retains its pending/leased meaning, so it is not the inverse of `isTerminal`. Use `!isTerminal` to identify unfinished runs. Auto-resume prefers leased, then pending, then waiting runs. `run:waiting` updates tracked runs and run lists; subscriptions hydrate waiting state after reconnect. Cancellation remains available while waiting; delete/retrigger require cancellation first. Inspect the persisted outcome and timing through the core `getWait()` / `getWaits()` APIs or the HTTP handler’s `GET /wait?runId=...&waitId=...` and `GET /waits?runId=...` routes. On reconnect, re-read the saved run and waits; SSE notifications may have been missed. Submit authorized input with `POST /signal?runId=...&waitId=...`.

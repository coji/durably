...
- **Events follow their state writes directly**: `run:fail` and `run:cancel` are now emitted immediately after the status write, as `run:complete` already was. Before, code polling storage could see `failed` or `cancelled` a storage round trip before the event. `run:fail` now names the failed step from the failures recorded under the current lease instead of reading attempts back. The ordering is documented and tested on SQLite, PostgreSQL and the browser (#220).
- **Unregistered jobs**: a run whose job is not registered in the worker now emits `run:leased` and `run:fail` (with `failedStepName: 'unknown'`), like any other failed run. It is also cleaned up like one: with `preserveSteps: false` its checkpoints and logs are deleted (#220).
- **`cancel()` cleanup**: a failure to delete checkpoints after a committed cancellation is reported as `worker:error` (`context: 'cancel-cleanup'`) instead of rejecting `cancel()` (#220).
- **`onError` isolation**: an exception thrown by the `onError` handler, or a rejected promise from an async handler, is now ignored. Before, it escaped the emitting call (for example rejecting `cancel()` after a committed cancellation) and stopped delivery to the remaining listeners, and for an async listener it became an unhandled rejection (#220).
- **Stable run and log ordering**: `getRuns()` and `storage.getLogs()` break creation-time ties by ID, so runs or logs written in the same millisecond keep a deterministic order and `limit`/`offset` pages no longer overlap or skip (#219).
...
- **Durable external waits**: Prepare and resolve persistent input addresses, suspend without occupying a worker slot or concurrency key, and resume the same run by checkpoint replay. Includes optional deadlines, input-wait and execution-slot timing, cancellation, idempotent signal receipts, direct wait queries, and waiting-aware active coalescing (#203, #204).
...
- **Breaking: Node.js 22+ required** for the published core package. The bundled `better-sqlite3` driver moves to v13 (#189).
- **Kysely compatibility**: The peer dependency now supports 0.27, 0.28, and 0.29 (#189).

#### @coji/durably-react

- The optional `@coji/durably` peer now requires `^0.16.0` when using the core package; `docs/llms.md` is included in the npm package (#208).

### Fixed

#### @coji/durably

- SSE responses now set `Cache-Control: no-cache, no-transform` so compression middleware forwards events without leaving fullstack clients showing a completed run as pending (#189).

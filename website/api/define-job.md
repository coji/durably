...
| `coalesce`          | `'skip' \| 'queue'`: return existing pending run instead of `ConflictError`, or create one trailing pending run if an active run is leased. Requires `concurrencyKey`. In this release, `'skip'` and `'queue'` are behaviorally equivalent aliases. At most one trailing pending run is maintained; further triggers coalesce onto it without persisting new inputs or labels (unused inputs are reported in `run:coalesced`). If `idempotencyKey` matches, disposition is `'idempotent'` (idempotency takes priority). If a leased run's lease expires while a pending replacement exists, the expired run fails and the replacement remains pending. |

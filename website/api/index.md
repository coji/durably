...
Prepare a durable input address with `step.prepareWait(name, { metadata?, timeoutMs? })`, start external work in a named step, then `await step.waitFor(wait)`. Deliver JSON input through `durably.signal(wait.id, payload, { signalId })`. A fixed deadline can instead produce `{ type: 'timeout' }`; a signal produces `{ type: 'signal', payload }`. Waiting releases execution capacity and resumes the same run by checkpoint replay. Direct wait queries expose deadline, outcome, and separate external-input and post-result-slot wait times. See [Step](./step#durable-external-waits), [direct wait methods](./create-durably#durable-wait-methods), and [HTTP wait routes](./http-handler#durable-waits-over-http).

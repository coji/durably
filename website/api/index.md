...
HTTP endpoints (`/runs`, `/run`) return `ClientRun` — the same fields minus `idempotencyKey`, `concurrencyKey`, `leaseExpiresAt`, and `updatedAt`. Use `toClientRun(run)` to apply the same projection in custom code.
